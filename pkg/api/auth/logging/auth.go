...
func New(svc auth.Service, logger authapi.Logger) *LogService {
	return &LogService{
		Service: svc,
		logger:  logger,
	}
...
func (ls *LogService) Authenticate(c echo.Context, user, password string) (resp authapi.AuthToken, err error) {
...
// Me logging
func (ls *LogService) Me(c echo.Context) (resp authapi.User, err error) {
	defer func(begin time.Time) {
		ls.logger.Log(
			c,
			name, "Me request", err,
