...
import (
	...
	"github.com/jpurdie/authapi/pkg/api/user/transport"
...
func TestCreate(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			rbac: &mock.RBAC{
				AccountCreateFn: func(c echo.Context, roleID authapi.AccessRole, companyID, locationID int) error {
...

		{
			...
			udb: &mockdb.User{
				CreateFn: func(db orm.DB, usr authapi.User) (authapi.User, error) {
					usr.ID = 1
...
func TestList(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			udb: &mockdb.User{
				ListFn: func(db orm.DB, q *authapi.ListQuery, p authapi.Pagination) ([]authapi.User, error) {
		...
		},
		{
			...
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, id int) (authapi.User, error) {
					return authapi.User{
						Role: &authapi.Role{
							AccessLevel: authapi.CompanyAdminRole,
						},
					}, nil
				},
			},
