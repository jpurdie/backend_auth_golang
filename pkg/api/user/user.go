...
func (u User) Create(c echo.Context, req authapi.User) (authapi.User, error) {
...
func (u User) List(c echo.Context, p authapi.Pagination) ([]authapi.User, error) {
	au := u.rbac.User(c)
	q, err := query.List(au)
	if err != nil {
		return nil, err
...
func (u User) View(c echo.Context, id int) (authapi.User, error) {
...
func (u User) Update(c echo.Context, r Update) (authapi.User, error) {
	if err := u.rbac.EnforceUser(c, r.ID); err != nil {
		return authapi.User{}, err
	}

	if err := u.udb.Update(u.db, authapi.User{
