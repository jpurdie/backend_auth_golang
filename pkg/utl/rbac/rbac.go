...
func (s Service) User(c echo.Context) authapi.AuthUser {
	id := c.Get("id").(int)
	companyID := c.Get("company_id").(int)
	user := c.Get("username").(string)
	email := c.Get("email").(string)
	role := c.Get("role").(authapi.AccessRole)
...
func (s Service) EnforceRole(c echo.Context, r authapi.AccessRole) error {
	return checkBool(!(c.Get("role").(authapi.AccessRole) > r))
}
...
func (s Service) EnforceCompany(c echo.Context, ID int) error {
	...
	if err := s.EnforceRole(c, authapi.CompanyAdminRole); err != nil {
		return err
	}
...
func (s Service) EnforceLocation(c echo.Context, ID int) error {
	...
	if err := s.EnforceRole(c, authapi.LocationAdminRole); err != nil {
		return err
	}
...
func (s Service) isAdmin(c echo.Context) bool {
	return !(c.Get("role").(authapi.AccessRole) > authapi.AdminRole)
}

func (s Service) isCompanyAdmin(c echo.Context) bool {
	// Must query company ID in database for the given user
	return !(c.Get("role").(authapi.AccessRole) > authapi.CompanyAdminRole)
}
...
func (s Service) AccountCreate(c echo.Context, roleID authapi.AccessRole, companyID, locationID int) error {
...
func (s Service) IsLowerRole(c echo.Context, r authapi.AccessRole) error {
	return checkBool(c.Get("role").(authapi.AccessRole) < r)
}
